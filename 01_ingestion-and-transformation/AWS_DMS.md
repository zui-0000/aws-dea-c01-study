...
| **レプリケーションインスタンス** | 実際に移行処理を行うEC2インスタンス（※DMS Serverlessではインスタンス管理は不要） |
| **ターゲットエンドポイント** | 移行先DBの接続情報 |
...
レプリケーションインスタンス（EC2）のサイズ選択・管理が不要な形態。
必要な容量を**自動プロビジョニング・自動スケール**するため、ワークロードに応じてキャパシティが調整される（2023年GA、2025年4月にストレージの自動スケールも追加）。

```
【従来型（プロビジョン型）】
レプリケーションインスタンスのサイズを自分で選定・管理

【DMS Serverless】
インスタンス管理不要 → 必要なキャパシティを自動プロビジョニング・自動スケール
```
...
なお、SCTデスクトップ版とは別に、**DMS Schema Conversion（DMS SC）** というDMSコンソールに統合されたフルマネージドなスキーマ変換機能も提供されている。
...
- **DMS Serverless** = インスタンス管理不要・容量を自動プロビジョニング/自動スケール
- **DMS Schema Conversion（DMS SC）** = DMSコンソール統合のフルマネージドなスキーマ変換機能
