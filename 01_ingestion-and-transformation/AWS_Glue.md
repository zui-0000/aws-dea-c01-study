...
Glueジョブは実行に使うバージョンを選択する。バージョンごとに同梱されるSpark・Python・Scala・Javaが異なる。

2026年6月時点の現役は **Glue 3.0 / 4.0 / 5.0 / 5.1**。

| バージョン | Apache Spark | Python | 備考 |
|--|--|--|--|
| **Glue 5.0**（2024年12月GA） | 3.5.4 | 3.11 | Scala 2.12.18・Java 17。Glue 4.0比で性能向上・コスト削減 |
| **Glue 5.1** | 3.5.6 | 3.11 | 5.0の後継 |

- **Glue 2.0 / 1.0 / 0.9 は2026年4月1日にサポート終了**（EOL）。新規は5.0以降を選ぶ。

---

## ワーカータイプ

Glue（Spark）ジョブはワーカータイプでリソースを指定する。1ワーカー = 一定のDPU・vCPU・メモリ。

| ワーカータイプ | DPU | vCPU | メモリ |
|--|--|--|--|
| **G.1X** | 1 | 4 | 16 GB |
| **G.2X** | 2 | 8 | 32 GB |
| **G.4X** | 4 | 16 | 64 GB |
...
- さらに大規模向けに **G.12X / G.16X**、メモリ最適化の **R型ワーカー** もある。
...
- **Glueバージョン** → 現行は5.0が中心（Spark 3.5.4・Python 3.11・Scala 2.12.18・Java 17、後継に5.1）。**Glue 2.0 / 1.0 / 0.9 は2026年4月1日にサポート終了**。新規は5.0以降
- **ワーカータイプ** → G.1X(4 vCPU/16GB)・G.2X(8 vCPU/32GB)・G.4X(16 vCPU/64GB)・G.8X(32 vCPU/128GB)。大規模はG.12X/G.16X・R型ワーカー
...
- **Flex 実行クラス** → 余剰リソースを使う。最大34〜35%安いが起動遅延・中断リスクあり。非緊急バッチ向け
