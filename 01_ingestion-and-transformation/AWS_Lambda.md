...
| **メモリ** | 最大10GB |
| **一時ストレージ（/tmp）** | 最大10GB |
| **デプロイパッケージ（ZIP直接アップロード）** | 50MB（圧縮時、コンソール/API/SDK経由） |
| **デプロイパッケージ（S3経由のZIP）** | それ以上も可 |
| **デプロイパッケージ（解凍後・レイヤー含む）** | 250MB |
| **デプロイパッケージ（コンテナイメージ）** | 最大10GB（ECR保存） |

**15分のタイムアウトが最も重要**。長時間かかる処理には使えない。

デプロイパッケージのサイズはアップロード方法で異なる。ZIPをコンソール/API/SDKで直接アップロードする場合は圧縮時で50MBまでだが、S3経由ならそれ以上も可。解凍後（レイヤー含む）は250MBが上限。コンテナイメージは最大10GB（ECRに保存）。
...
| | ZIP ファイル | コンテナイメージ |
|--|--|--|
| サイズ上限 | 直接アップロード50MB（圧縮時）／解凍後250MB（レイヤー含む。S3経由なら圧縮時はそれ以上も可） | 最大10GB |
...
アカウントのデフォルト同時実行数は**リージョンあたり1,000**（ソフトリミット。引き上げ申請が可能）。
...
- **デプロイパッケージのサイズ** → ZIP直接アップロードは50MB（圧縮時）・S3経由ならそれ以上も可・解凍後（レイヤー含む）は250MB・コンテナイメージは最大10GB
- **デフォルト同時実行数は1,000**（リージョンあたり・ソフトリミット・引き上げ申請可）
