...
- シャード数のデフォルト上限は**2025年4月に 20,000シャード/アカウント に引き上げ**られた（上限緩和申請でさらに拡張可能）

---

## 容量モード（キャパシティモード）

Kinesis Data Streams には2つの容量モードがある。シャードを自分で管理するか、自動スケールに任せるかの違い。

| 観点 | プロビジョンドモード | オンデマンドモード |
|------|-------------------|------------------|
| シャード管理 | 手動でシャード数を指定・調整 | 不要（自動スケール） |
| スケーリング | 自分でシャードを増減 | トラフィックに応じて自動拡張 |
| 容量計画 | 必要 | 不要 |
| 課金 | プロビジョニングしたシャード時間 | 取り込み・取得したデータ量 |
| 向いているケース | 負荷が予測可能・安定したワークロード | 突発的・予測困難なワークロード |

### プロビジョンドモード

シャード数を**手動で管理**する従来型のモード。前述の「シャード」セクションの説明（1シャード = 書き込み1MB/秒・1,000レコード/秒、読み取り2MB/秒）はこのモードのもの。負荷の増減に合わせて自分でシャードを追加・削除する必要がある。

### オンデマンドモード

シャードを意識せず、トラフィックに応じて**自動でスケール**するモード。容量計画が不要で、突発的な負荷に強い。

```
初期キャパシティ: 書き込み 4MB/秒・4,000レコード/秒
    ↓ トラフィック増加に応じて自動拡張
デフォルト上限: 200MB/秒
    ↓ 上限緩和申請でさらに拡張
最大: 書き込み 10GB/秒・読み取り 20GB/秒
```
...
| スケーリング | プロビジョンド（手動）/ オンデマンド（自動）の2モード | 自動スケーリング |
| 管理コスト | 高い | 低い |
...
- **オンデマンドモード vs プロビジョンドモード** → 突発的・予測困難な負荷はオンデマンド（自動スケール・容量計画不要）、予測可能で安定した負荷はプロビジョンド（手動シャード管理）
