...
キャパシティは **RPU（Redshift Processing Unit）** 単位で指定し、最小容量は **4 RPU**（2025年に8 RPU → 4 RPU へ引き下げ）。

### AQUA（Advanced Query Accelerator）

コンピュートノードの近くにキャッシュ層を持ち、クエリを高速化する仕組み。

> **注意:** ユーザーが手動で有効化する独立した高速化機能としては**廃止済み**。設定用の `ModifyAquaConfiguration` API も retired（呼び出してもAQUA構成は変更されない）であり、現在は **Redshiftが使用要否を自動的に判断**する。利用者が明示的にオン/オフを切り替えるものではない。

### ノード種別（RA3 / DC2）

| ノード種別 | 概要 | 状況 |
|-----------|------|------|
| **RA3**（ra3.xlplus / ra3.4xlarge / ra3.16xlarge） | コンピュートとストレージを分離し、**Managed Storage**（S3ベース）でストレージを自動拡張。ノード数とは独立してストレージを拡張できる | **現行** |
| **DC2**（dc2.large / dc2.8xlarge） | コンピュートとローカルSSDストレージが一体型 | **2026年4月24日にEOL**（RA3 / Serverless への移行が推奨） |

### Concurrency Scaling（同時実行スケーリング）

同時実行クエリで高負荷になったときに、**一時的なクラスタを自動で追加**して読み取り/書き込みの処理能力を拡張する機能。負荷が下がると自動で縮退する。

- 1日あたり**最大1時間分の無料クレジット**が付与される
- 無料クレジットは**最大30時間まで累積**できる
...
- **AQUA** → 手動で有効化する機能としては廃止済み（`ModifyAquaConfiguration` も retired）。現在は**Redshiftが使用要否を自動判断**する
- **ノード種別** → **RA3 が現行**（コンピュートとストレージを分離・Managed Storage）。**DC2 は2026年4月24日にEOL**
- **Concurrency Scaling** → 同時実行の高負荷時に一時クラスタを自動追加。1日最大1時間分の無料クレジット（最大30時間まで累積）
- **Redshift Serverless** → 最小容量は **4 RPU**（2025年に8→4へ引き下げ）
